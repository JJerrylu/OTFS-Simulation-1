...
% H_dd generator
% ----------------------------
function [H_dd,delay_p, doppler_p, gain_p ] = generate_dd_channel(K, L, P, Ts, tau_max, nu_max,linearpowProfile)
    ...
    for p = 1:P
        ...
        if p==1
            ...
            delay_p(p) = randi([1 alpha_max]); %Integer delay
        end
        % Doppler shift index
        theta   = rand()*2*pi - pi;  % θ ∈ [−π, π]
        nu_p    = nu_max * cos(theta);
        doppler_p(p)     = round(K *L* Ts * nu_p);  % quantized Doppler shift index

        % path gain
        gain_p(p)    = linearpowProfile(p);
        phase_shift_p(p) = exp(1i*2*pi*delay_p(p)*doppler_p(p));
...
for idx = 1:length(snr_dB_range)
    ...
    for trial = 1:num_iter
        ...
        [H_dd,delay_p, doppler_p, gain_p] = generate_dd_channel(K, L, P, Ts, tau_max, nu_max,linearpowProfile);

        %% === 3. DZT-OTFS tranceiver ===
        tx_dzt = dzt_otfs_modulate(Zx);
        Zy_dzt = H_dd * Zx(:);  % perfect CSI model
        noise  = sqrt(noise_var/2)*(randn(size(Zy_dzt)) + 1j*randn(size(Zy_dzt)));
        Zy_dzt = Zy_dzt + noise;
        
        %% === 4. 2-step OTFS tranceiver ===
        tx_2step = twostep_otfs_modulate(Zx);
        Zy_2step = H_dd * Zx(:);
        Zy_2step = Zy_2step + noise;
...
figure; surf(abs(H_dd),'FaceColor','interp'); title('DD domain channel matrix H\_dd (abs)'); xlabel('delay'); ylabel('doppler');
